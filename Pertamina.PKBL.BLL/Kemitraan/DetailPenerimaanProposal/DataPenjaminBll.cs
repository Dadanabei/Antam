...
namespace Pertamina.PKBL.BLL.Kemitraan.DetailPenerimaanProposal
{
    public class DataPenjaminBll
    {
        ...
        public DataTable getData(Hashtable parameters)
        {
            try
            {
                return dbHelper.GetData("[USP_KEMITRAAN_SELECT_DATA_PENJAMIN]", parameters);
            }
            catch (Exception exc)
            {
                throw exc;
        ...
        public void updateData(Hashtable parameters)
        {
            try
            {
                dbHelper.ExecuteNonQuery("[USP_KEMITRAAN_ENTRI_DATA_PENJAMIN]", parameters);
            }
            catch (Exception exc)
            {
                throw exc;
